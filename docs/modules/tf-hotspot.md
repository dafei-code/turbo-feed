...
> 热点探测与治理 SDK。**进程内库**（不独立部署）：按设计嵌入 tf-counter / tf-feed-engine 进程使用。
> **当前状态：模块骨架，业务未实现**（源文件 0 个；pom 依赖已就位，且已被 tf-counter / tf-feed-engine 声明依赖，但**尚无任何代码调用**）。本文档**通篇是设计蓝图，不是现状描述**。
