 ...
 * 否则同一 mediaId 的 append 与 remove 会落到不同队列、失去顺序保证，出现
 * 「remove 先执行、append 后执行 → 已下架内容重新出现」的内容安全事故。</p>
 *
 * <p><b>顺序性</b>：{@code consumeMode = ORDERLY} + 发送侧 {@code syncSendOrderly(..., hashKey=mediaId)}，
 * 保证同一 mediaId 的事件严格按序消费。</p>
 *
    ...
    public void onMessage(FeedTimelineEvent event) {
        try {
            if (FeedTimelineEvent.ACTION_APPEND.equals(event.action())) {
                ...
                feedTimelineStore.removeStrict(event.mediaId());
            } else {
                ...
                throw new IllegalArgumentException("未知的 Feed 时间线 action（拒绝按 REMOVE 兜底）: action="
                        + event.action() + ", mediaId=" + event.mediaId());
            }
        } catch (Exception e) {
            // 抛异常 → RocketMQ 按 maxReconsumeTimes 重投；重投耗尽进入死信队列 %DLQ%{consumerGroup}，
            // 由人工 / 巡检对账处理。严格写入入口保证异常能冒泡到此（fail-open 路径会吞掉）。
            log.error("Feed 时间线消费失败，触发 RocketMQ 重试/DLQ: mediaId={}, action={}",
                    event.mediaId(), event.action(), e);
            throw e;
        }
    }
}
